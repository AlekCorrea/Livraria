...
class Livro {
    constructor({ id = null, titulo, autor, categoria, ano, editora, numeroPaginas }) {
        ...
        this.editora = editora ? String(editora).trim() : null;
        this.numeroPaginas = numeroPaginas ? Number(numeroPaginas) : null;
        this._validar();
    }
    ...
    _validar() {
        ...
        if (this.numeroPaginas !== null && (!Number.isInteger(this.numeroPaginas) || this.numeroPaginas <= 0)) {
            erros.push('numeroPaginas deve ser um número inteiro positivo');
        }
        if (erros.length > 0) {
